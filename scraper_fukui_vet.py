...
データソース（seed_fukui_vet.json に事前収集して同梱）:
  - 福井県獣医師会 会員病院一覧（fukuijuu.com/hospital/）
  - トリムトリム（trimtrim.jp）福井県・エキゾチックアニマル対応病院検索
  - ホームメイト・リサーチ（homemate-research-pet-clinic.com）福井県一覧（全2ページ）
  - 上記に無かった数院は個別にネット検索して住所・電話番号を補完

処理:
  Phase 1 - 各ソースを病院名（法人格・支店表記等を除いた主要部）で統合・重複排除。
            電話番号・URLが判明している値を優先して残す。
  Phase 2 - 既知の公式サイトURLから メール・インスタ・問い合わせフォームURL・
            （未判明なら）電話番号 を取得する。
  Phase 3 - 公式サイトURLが未判明の病院について DuckDuckGo で公式サイトを検索し、
            電話番号一致または病院名一致で検証する。
  Phase 4 - 電話番号は福井県の市外局番（0776/0770/0778/0779、いずれも4桁）で
            ハイフン整形する。既にハイフン区切りの表記はそのまま採用する。

出力列: 名称, メールアドレス, 公式サイトURL, 所在地, 電話番号, インスタURL, 問い合わせフォームURL
出力ファイル: 福井県_動物病院リスト.xlsx
"""
...
LOG_FILE = "scraper_fukui_vet.log"
...
# ── 定数 ─────────────────────────────────────────────────
OUTPUT_FILE = "福井県_動物病院リスト.xlsx"
SEED_JSON = "seed_fukui_vet.json"
PREF_NAME = "福井県"
UA = (
...
SKIP_DOMAINS = [
    ...
    "web-kanji.com", "map.yahoo.co.jp", "value-domain.com", "onamae.com",
    "houjinbase.com", "houjin.jp", "baseconnect.in",
    # 今回の元データ自体（病院の公式サイトではない）
    "fukuijuu.com", "trimtrim.jp", "homemate-research-pet-clinic.com",
    # 動物病院の口コミ・予約・検索ポータル（公式サイトではない）
    "pet.caloo.jp", "ipetclub.jp", "anicom-ah.com", "epark.jp", "petlife.asia",
    "doctorsfile.jp", "navita.co.jp", "localplace.jp", "1-2-pet.com",
    "pfirst.jp", "cast-a-net.net", "kruz.co.jp", "qlife.jp", "ozmall.co.jp",
    # 検証したが誤検出と判明したドメイン（地域ポータル／記事等）
...
SKIP_URL_PATTERNS = [
    r"\.pdf(\?|$)",
    r"/company/\d+",
    r"://city\.[a-z0-9-]+\.fukui\.jp",  # 市町村公式サイト（病院の公式サイトではない）
    r"://[a-z0-9.-]*\.local-now\.jp",   # 地域ニュース・ポータル記事
    r"://[a-z0-9.-]*b-mall\.ne\.jp",    # ビジネスモール（企業情報ディレクトリ）
]
...
def _name_core(name: str) -> str:
    # 「山下動物病院」と「山下動物病院敦賀分院」のように分院は別施設として区別する
    # ため、法人格・読み仮名の除去のみ行い、支店/分院表記は残したまま正規化する。
    core = re.sub(r"(株式会社|有限会社|合同会社|合資会社|一般社団法人|特定非営利活動法人|NPO法人|医療法人)", "", str(name))
...
# 福井県で使用される市外局番はすべて4桁で、桁の重複が無いため所在地に
# 頼らず数字だけで判定できる（048/049型の3桁と4桁が混在する県と異なる）。
#   0776 : 福井市・あわら市・坂井市・吉田郡永平寺町
#   0770 : 敦賀市・小浜市・三方郡美浜町・三方上中郡若狭町・大飯郡高浜町/おおい町
#   0778 : 鯖江市・越前市・丹生郡越前町・今立郡池田町・南条郡南越前町
#   0779 : 大野市・勝山市
MOBILE_PREFIX = ("070", "080", "090", "050")
FREE_PREFIX_4 = ("0120", "0800", "0570")
FUKUI_AREA_4 = ("0776", "0770", "0778", "0779")

CITY_AREA: dict[str, str] = {}
for _cities, _code in (
    (["福井市", "あわら市", "坂井市", "永平寺"], "0776"),
    (["敦賀", "小浜", "三方", "若狭町", "高浜", "おおい"], "0770"),
    (["鯖江", "越前市", "越前町", "池田町", "南越前"], "0778"),
    (["大野市", "勝山"], "0779"),
):
    for _c in _cities:
        CITY_AREA[_c] = _code
...
def format_phone(raw: str, address: str = "") -> str:
    ...
    if len(d) == 10:
        ...
        # 福井県の市外局番はすべて4桁（0776/0770/0778/0779）で重複が無いため、
        # 数字自体から素直に判定できる。県外番号（0120等除く一般4桁市外局番）も
        # 同じ 4-2-4 の割り方で妥当なことが多いため、それを既定とする。
        if d.startswith(FUKUI_AREA_4):
            return f"{d[:4]}-{d[4:6]}-{d[6:]}"
        area = _area_from_address(address)
        if area and d.startswith(area):
            return f"{area}-{d[4:6]}-{d[6:]}"
        if d[:2] in ("04",) and d[2] in "2367":  # 所沢等 04-XXXX-XXXX 系の県外番号
...
def load_and_merge_seed() -> list[dict]:
    logger.info(f"Phase 1: {SEED_JSON}（福井県獣医師会 / トリムトリム / ホームメイト）を統合")
