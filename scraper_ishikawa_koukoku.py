...
データソース:
  Phase 1 - NAVITIME 業種カテゴリ「広告代理店」(石川県)
            https://www.navitime.co.jp/category/0516001/17/?tags=010429
            全7ページ（約99件）。一覧に 名称・住所・電話番号 が掲載されている。
            電話番号は数字のみで表記されているため、市外局番の桁数に応じて
            ハイフンを補い「的確な表示」（例: 076-221-4714）に整形する。
  Phase 2 - Baseconnect「石川県の広告業界の会社」一覧（baseconnect_ishikawa_koukoku.json）
            電話番号は非公開（サイト上は「―」表示）だが、企業詳細ページに
            公式サイトURL・Instagram URL が掲載されている。会社名で Phase 1 に
            突き合わせて 公式サイトURL 等を補完し、Phase 1 未収録で「広告業界」を
            主業界とする会社は新規追加する。
  Phase 3 - 公式サイトURLが未判明の事務所について DuckDuckGo で公式サイトを検索し、
            電話番号一致または会社名一致で検証する。
  Phase 4 - 判明した公式サイトから メール・インスタ・問い合わせフォームURL を取得する。

出力列: 名称, メールアドレス, 公式サイトURL, 所在地, 電話番号, インスタURL, 問い合わせフォームURL
出力ファイル: 石川県_広告代理店リスト.xlsx
"""
...
LOG_FILE = "scraper_ishikawa_koukoku.log"
...
# ── 定数 ─────────────────────────────────────────────────
OUTPUT_FILE = "石川県_広告代理店リスト.xlsx"
BASECONNECT_JSON = "baseconnect_ishikawa_koukoku.json"
UA = (
...
NAVITIME_BASE = "https://www.navitime.co.jp/category/0516001/17/"
NAVITIME_TAG = "010429"  # 広告代理店
NAVITIME_PAGES = 7
...
def _name_core(name: str) -> str:
    core = re.sub(r"(株式会社|有限会社|合同会社|合資会社)", "", str(name))
    core = re.sub(r"(北陸\s*(支社|支店|営業所|支局|営業部)|金沢\s*(支社|支店|営業所|支局))$", "", core)
...
# 石川県で使用される市外局番（長い順に判定）
ISHIKAWA_AREA_4 = ("0761", "0767", "0768")  # 小松/加賀・七尾/羽咋・輪島/珠洲
MOBILE_PREFIX = ("070", "080", "090", "050")
FREE_PREFIX_4 = ("0120", "0800", "0570")


def format_phone(raw: str) -> str:
    """数字のみ／記号混じりの電話番号を市外局番桁数に応じてハイフン整形する。"""
    d = _normalize_phone(raw)
    if not d:
        return ""
    # 国番号 +81 / 81 始まりを 0 始まりに戻す
    if d.startswith("81") and len(d) >= 11:
        d = "0" + d[2:]
    if len(d) == 10:
        if d.startswith(FREE_PREFIX_4):
            return f"{d[:4]}-{d[4:7]}-{d[7:]}"
        if d.startswith(ISHIKAWA_AREA_4):
            return f"{d[:4]}-{d[4:6]}-{d[6:]}"
        if d.startswith("076"):  # 金沢・白山・野々市・かほく 等
...
# ── Phase 1: NAVITIME 広告代理店カテゴリ ──────────────────

def scrape_navitime() -> list[dict]:
    logger.info("Phase 1: NAVITIME「広告代理店」(石川県) を収集")
    records: list[dict] = []
    for page in range(1, NAVITIME_PAGES + 1):
        ...
        for li in items:
            ...
            phone = format_phone(phone_m.group(1)) if phone_m else ""
            if not name:
                continue
...
def scrape_baseconnect(records: list[dict]) -> None:
    ...
    for i, row in enumerate(rows, 1):
        ...
        is_ad_primary = row.get("industries", "").startswith("広告業界の会社")
...
def search_official_site(name: str, address: str, phone: str) -> tuple[str, str]:
    city_m = re.search(r"石川県\s*([^\s0-9]{2,8}?[市区町村郡])", address)
    city = city_m.group(1) if city_m else "石川県"
    query = f"{name} {city} 広告 公式サイト"
    results = []
